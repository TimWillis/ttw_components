...
export class DiffFinder {
    constructor(t1Node, t2Node, options) {
        ...
        this.t1 =
            t1Node instanceof HTMLElement
                ? nodeToObj(t1Node, this.options)
                : typeof t1Node === 'string'
                ? stringToObj(t1Node, this.options)
                : JSON.parse(JSON.stringify(t1Node));
        this.t2 =
            t2Node instanceof HTMLElement
        ...
        do {
            if (this.options.debug) {
                ...
                if (this.diffcount > this.options.diffcap) {
                    ...
                    throw new Error(
                        `surpassed diffcap:${JSON.stringify(this.t1Orig)} -> ${JSON.stringify(this.t2Orig)}`,
                    );
                }
            }
    ...
    findOuterDiff(t1, t2, route) {
        ...
        if (
            route.length &&
            this.options.maxNodeDiffCount < Math.abs((t1.childNodes || []).length - (t2.childNodes || []).length)
        ) {
            return [
                new Diff()
                    .setValue(this.options._const.action, this.options._const.replaceElement)
    ...
    findValueDiff(t1, t2, route) {
        ...
        t2.value = t2.value || (t2.attributes && t2.attributes.value) || '';
        ...
        t2.checked = t2.checked || (t2.attributes ? t2.attributes.checked === '' && true : undefined) || undefined;
