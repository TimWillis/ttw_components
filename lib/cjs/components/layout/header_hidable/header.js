...
const convert_style_obj_1 = __importDefault(require("../../../utilities/convert_style_obj"));
const touch_1 = __importDefault(require("../../../utilities/touch"));
const css = /*css*/ `
    header{
        width: 100%;
        color: red;
...
const html = (props) => {
    return /*html*/ `
    <header>
        <div id="header_inner_container" class='layout horizontal center-center' style=${(0, convert_style_obj_1.default)(props.styles.header)}>
            <div class='logo'>LOGO
            </div>
            <div class="flex"></div>
        ...
        .join('')}
            ...
            <div class="mobile_nav">
                ...
                <nav class="layout vertical center start-justified ${props.open ? 'open' : 'close'}">                    
                    ${props.pages
        .map(function (page) {
        return `<a  class="layout horizontal center-center" href='#${page.url}'>${page.name}</a>`;
    })
...
const header = (selector, parent_app, pages) => {
    let clicked = false, open = false, header_shown = true;
    ...
    const setup_events = () => {
        ...
        document.querySelector('.mobile_nav nav').addEventListener('click', (event) => {
            //   console.log("test1");
            event.stopImmediatePropagation();
            store.data = toggle(store.data);
        });
        if (window.innerWidth < 1080) {
            setTimeout(() => {
                ...
                (0, touch_1.default)(header, swiped);
            }, 4000);
        }
    };
