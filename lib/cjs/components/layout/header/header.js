...
const mobile_nav = (state) => {
    return `    
        ...
        <nav id='nav_list' class="layout vertical center start-justified ${state.open ? 'open' : 'close'}" style="display: ${state.styles.nav_display}">                    
            ${state.pages
        .map(function (page) {
    ...
    <header>
        <div id="header_inner_container" class='layout horizontal center-center fancy_font'>
            <div style="height: 100%;">
                <img id="logo" alt="${state.name} Logo" style="height: 75px; width: 78px;" src="${state.logo_url}"/>
            </div>
            <h1 class='logo'>
                ${state.name}
            </h1>
...
const header = (pages, name, logo_url) => {
    // const store = {
    //     open: false
    // }
    ...
    const open_close = () => {
        ...
        (0, dom_diffing_1.default)('mobile_nav', mobile_nav(state), 'div', undefined, true);
    };
